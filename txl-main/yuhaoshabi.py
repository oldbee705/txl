...
class txl:
    def __init__(self):
        ...
        self.rt.place(x = 240, y = 10)

        #创建列表
        self.tree_txl = ttk.Treeview(
            self.lf,
            show = 'headings', #创建表头
            columns = ('name', 'phone', 'address'), #创建列名（属性）
            height= 30,
            )
        #定义属性的名称
    ...
    def delete_achieve(self):
        ...
        try:
            cursor.execute('''
                ...
                WHERE name = ?;
            ''',(self.name,)  #','非常重要;使它成为元组
            )
            conn.commit()
            self.delete_w.destroy()
            self.overload()
        except:
    ...
    def update(self):
        self.update_w = tk.Tk()
        self.update_w.title('Update')
        self.update_w.geometry('300x230+500+450')

        self.label = tk.Label(self.update_w,text = 'Name to be change:').pack()
        self.update_name = tk.Entry(self.update_w)
        self.update_name.pack()

        self.label = tk.Label(self.update_w,text = 'new name:').pack()
        self.updated_name = tk.Entry(self.update_w)
        self.updated_name.pack()

        self.label = tk.Label(self.update_w,text = 'new phone:').pack()
        self.updated_phone = tk.Entry(self.update_w)
        self.updated_phone.pack()

        self.label = tk.Label(self.update_w,text = 'new address:').pack()
        self.updated_address = tk.Entry(self.update_w)
    ...
    def select(self):
        self.select_w = tk.Tk()
        self.select_w.title('Select')
        self.select_w.geometry('300x100+500+730')

        self.label_name = tk.Label(self.select_w,text = 'name:').pack()
        self.select_name = tk.Entry(self.select_w)
